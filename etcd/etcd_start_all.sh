...
for i in "${nodes[@]}"
do
    ssh -i ~/.ssh/id_ed25519 -t $i 'sudo systemctl daemon-reload && sudo systemctl enable etcd && sudo systemctl start etcd.service && sudo systemctl status -l etcd.service'
done
